...
class UNet_mini(nn.Module):
    def __init__(self, num_classes, in_channels=1, bn=False):
        ...
        # Center Block
        self.center_conv1 = nn.Conv2d(256, 512, kernel_size=3, padding=1)
        self.center_norm1 = nn.InstanceNorm2d(512) if bn else nn.GroupNorm(32, 512)
        self.center_relu1 = nn.ReLU(inplace=True)
        self.center_conv2 = nn.Conv2d(512, 256, kernel_size=3, padding=1)
        self.center_norm2 = nn.InstanceNorm2d(256) if bn else nn.GroupNorm(32, 256)
        self.center_relu2 = nn.ReLU(inplace=True)
        self.center_conv_transpose = nn.ConvTranspose2d(256, 128, kernel_size=2, stride=2)
        ...
        self.final_conv1 = nn.Conv2d(256, num_classes, kernel_size=1)
        self.final_conv2 = nn.Conv2d(128, num_classes, kernel_size=1)
